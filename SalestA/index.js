...
function addToCartClicked(event) {
    ...
    var tax ;
    if(shopItem.classList.contains('tax-0')){
        tax=0;
    }
    else if(shopItem.classList.contains('tax-10')){
        tax = 10;
    }
    else if(shopItem.classList.contains('tax-15')){
        
        tax = 15;
    }
    addItemToCart(title, price, imageSrc,tax)
    updateCartTotal()
}

function addItemToCart(title, price, imageSrc,tax) {
    var cartRow = document.createElement('div')
    cartRow.classList.add('cart-row')
    cartRow.classList.add(`tax-${tax}`)
    var cartItems = document.getElementsByClassName('cart-items')[0]
...
function updateCartTotal() {
    ...
    for (var i = 0; i < cartRows.length; i++) {
        ...
        else if(cartRow.classList.contains('tax-15')){
